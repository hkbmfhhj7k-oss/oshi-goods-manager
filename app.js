...
const ANALYZER_URL = "https://oshi-goods-analyzer.kbwckqs79z.workers.dev";
// App AI pipeline v4.2: normal screenshots + full-page chunking; TOP owns product identity.
let sourceScanData = "";
let productImageData = "";
let aiRunning = false;
let showingSourceImage = false;
...
const AI_FIELD_IDS = ["series","title","price","orderStart","orderEnd","reserveDeadline","releaseDate","shippingText","shop","variantCount","purchaseLimit","boxPrice","boxCount"];
function clearAutoFillMarks(){AI_FIELD_IDS.forEach(id=>$(id)?.classList.remove("autofilled"));$("notes")?.classList.remove("autofilled");}
function setAutoField(id,value,opts={}){const el=$(id);if(!el||value===undefined||value===null||value==="")return false;if(!opts.overwrite&&String(el.value||"").trim()!=="")return false;el.value=value;el.classList.add("autofilled");return true;}
...
async function cropProductImage(src,box){const b=normalizeBox(box);if(!b)return"";const area=b.width*b.height;if(b.width<.28||b.height<.16||area<.06||area>.82)return"";const img=await loadImage(src);let sx=Math.round(b.x*img.width),sy=Math.round(b.y*img.height),sw=Math.round(b.width*img.width),sh=Math.round(b.height*img.height);sw=Math.min(sw,img.width-sx);sh=Math.min(sh,img.height-sy);if(sw<80||sh<80)return"";const px=Math.round(sw*.015),py=Math.round(sh*.015);sx=Math.max(0,sx-px);sy=Math.max(0,sy-py);sw=Math.min(img.width-sx,sw+px*2);sh=Math.min(img.height-sy,sh+py*2);const max=1200,scale=Math.min(1,max/sw,max/sh),c=document.createElement("canvas");c.width=Math.max(1,Math.round(sw*scale));c.height=Math.max(1,Math.round(sh*scale));c.getContext("2d").drawImage(img,sx,sy,sw,sh,0,0,c.width,c.height);return c.toDataURL("image/jpeg",.86);}
...
function mergeAiProducts(top,details=[]){
  // 主商品の身元はTOPだけを信頼する。DETAILは不足している補助情報だけを足す。
  const out={...top};
  const ds=details.filter(Boolean);
  const firstString=(key)=>ds.map(x=>x?.[key]).find(isUsefulString)||"";
  const firstPositive=(key)=>{const hit=ds.find(x=>Number(x?.[key])>0);return hit?hit[key]:null;};

  // TOP専用: series/title/price/releaseDate/shop/mainImage はDETAILから上書きしない。
  // DETAILで補完してよい項目だけを限定する。
  ["orderStart","orderEnd","reserveDeadline","shippingText","bonusText"].forEach(k=>{
    if(!isUsefulString(out[k])) out[k]=firstString(k);
  });
  ["variantCount","purchaseLimit","boxPrice","boxCount"].forEach(k=>{
    if(out[k]===null||out[k]===undefined||out[k]===""||Number(out[k])<=0) out[k]=firstPositive(k);
  });

  out.isRandom=out.isRandom===true||ds.some(x=>x?.isRandom===true);
  out.hasBonus=out.hasBonus===true||ds.some(x=>x?.hasBonus===true);
...
async function runAiAnalysis(){if(!sourceScanData||aiRunning)return;const status=$("aiStatus"),icon=$("aiStateIcon"),bar=$("aiProgressBar");$("aiPanel").classList.remove("hidden");aiRunning=true;status.classList.remove("ocr-error");status.classList.add("ai-working");status.textContent="商品ページを読んでいます…";icon.textContent="AI";bar.style.width="10%";$("aiResultSummary").textContent="フルページなら上部と詳細を分けて読み取ります。";$("aiCandidate").classList.add("hidden");try{
  ...
  let cropped="";
  if(conf>=.7)try{cropped=await cropProductImage(chunks.top.image,topData.mainImage);}catch{}
...
function resetForm(){
  ...
  sourceScanData=""; productImageData=""; showingSourceImage=false; clearAutoFillMarks();
  $("aiPanel")?.classList.add("hidden"); $("aiCandidate")?.classList.add("hidden"); $("toggleSourceBtn")?.classList.add("hidden");
  if($("aiResultSummary")) $("aiResultSummary").textContent=""; if($("aiProgressBar")) $("aiProgressBar").style.width="0%";
...
$("imageInput").onchange=(e)=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=async()=>{sourceScanData=r.result;productImageData="";editingImageData="";showPreview(sourceScanData,true);$("aiPanel").classList.remove("hidden");$("toggleSourceBtn").classList.add("hidden");await runAiAnalysis();};r.readAsDataURL(f);};
$("rerunAiBtn").onclick=()=>runAiAnalysis();
$("toggleSourceBtn").onclick=()=>{if(!sourceScanData||!productImageData)return;showPreview(showingSourceImage?productImageData:sourceScanData,!showingSourceImage);};
